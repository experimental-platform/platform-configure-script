...
function download_and_verify_image() {
  local image=$1
  echo -e "\tINSTALLER: Tagging old image"
  $DOCKER tag -f $image "$image-previous" 2>/dev/null || true # do not fail, this is just for backup reason
  echo -e "\tINSTALLER: Pulling Image"
  $DOCKER pull $image
  echo -e "\tINSTALLER: Validating all layers"
...
if [ "$(id -u)" != "0" ]; then
  echo "INSTALLER: Can not run without root permissions."
  exit 2
fi
...
if [[ -z "${CHANNEL}" ]]; then
  if [ -e ${CHANNEL_FILE} ]; then
    ...
    echo "INSTALLER: Using channel '${CHANNEL}' from ${CHANNEL_FILE}."
  else
    CHANNEL=stable
    echo "INSTALLER: No channel given. Using '${CHANNEL}' (default channel)."
  fi
else
  echo "INSTALLER: Using '${CHANNEL}' from the command line."
fi

echo "INSTALLER: FETCHING configure:${CHANNEL}"
download_and_verify_image $REGISTRY/configure:${CHANNEL}

echo "INSTALLER: Cleaning up and installing configuration"
...
if [[ ! ${available_channels} =~ ${CHANNEL} ]]; then
  echo "INSTALLER: We're on feature channel '${CHANNEL}'"
  CHANNEL=development
fi

# prefetch buildstep. so the first deployment doesn't have to fetch it.
echo "INSTALLER: FETCHING experimentalplatform/buildstep:latest"
...
echo "INSTALLER: FETCHING ibuildthecloud/systemd-docker"
...
for IMAGE in $IMAGES; do
  ...
  if [[ ! ${IMAGE} =~ "experimentalplatform/buildstep" ]]; then
    echo "INSTALLER: FETCHING ${IMAGE}"
    download_and_verify_image $IMAGE
  else
    echo "INSTALLER: NOT FETCHING ${IMAGE} ;)"
  fi
done
echo "INSTALLER: DONE FETCHING ALL IMAGES."

if [ "$RELOAD" = true ]; then
  echo "INSTALLER: Reloading systemctl after update."
  systemctl restart init-protonet.service
  exit 0
fi

if [ "$REBOOT" = true ]; then
  echo "INSTALLER: Rebooting after update."
